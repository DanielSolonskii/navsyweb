<template>
  <l-map ref="monitoring_map" style="height: 100%; width:100%; position:relative" :zoom="zoom"
    ...
    <map-info-control />
    <div class="top-right-leaflet-controls-container elevation-24">
      <!-- <v-geosearch :options="geosearchOptions"></v-geosearch>-->
      <l-control class="d-flex elevation-24">
        <div class="leaflet-custom-control" @click="toggleSearchBar">
          <v-tooltip bottom>
        ...
        <div class="leaflet-custom-control" @click="togglePlacesControl">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
        ...
        <div class="leaflet-custom-control" @click="toggleGeoZoneControl">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-icon v-bind="attrs" v-on="on">mdi-vector-square</v-icon>
            </template>
            <span>Геозоны</span>
          </v-tooltip>
        </div>
        <div class="leaflet-custom-control" @click="toggleMapProviderControl">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-icon v-bind="attrs" v-on="on">mdi-map</v-icon>
            </template>
            <span>Карта</span>
          </v-tooltip>
        </div>
    ...
    <l-control class="leaflet-custom-bottom-control" position="bottomright">
      <v-icon>mdi-star</v-icon>
    </l-control>
    <l-control class="leaflet-custom-bottom-control" position="bottomright">
      <v-icon @click="toggleRouting">mdi-road</v-icon>
    </l-control>
    ...
    <v-slide-y-transition>
      <l-control class="geozone-control elevation-4 pa-4 rounded" v-show="geoZoneControl">
        ...
        <div class="geo-buttons-block-add">
          <v-slide-x-transition>
            <v-tooltip left v-if="hover">
              <template v-slot:activator="{ on, attrs }">
                <v-btn @click="triggerCircle()" fab dark small color="white" v-bind="attrs" v-on="on"
                  class="absolute-add-geo-circle transition-effect">
          ...
          <v-tooltip left v-if="hover">
            <template v-slot:activator="{ on, attrs }">
              <v-btn @click="triggerPolygon" fab dark small color="white" v-bind="attrs" v-on="on"
                class="absolute-add-geo-voluntary" @mouseenter="triggerHover()" @mouseleave="removeHover()">
                <v-icon class="color-black">
                  mdi-code-brackets
                </v-icon>
              </v-btn>
            </template>
            <span>Новая геозона (Произвольная геозона)</span>
          </v-tooltip>

          <v-tooltip left v-if="hover">
            <template v-slot:activator="{ on, attrs }">
              <v-btn @click="triggerMarker()" fab dark small color="white" v-bind="attrs" v-on="on"
                class="absolute-add-geo-new-zone" @mouseenter="triggerHover()" @mouseleave="removeHover()">
                <v-icon class="color-black">
                  mdi-road
                </v-icon>
              </v-btn>
            </template>
            <span>Новая геозона (Маршрут)</span>
          </v-tooltip>
        </div>
    ...
    <v-slide-y-transition>
      <l-control class="geozone-control elevation-4 pa-4 rounded" v-if="openSearchBar">
        ...
        <div class="search-tool-results-header">
...
<script>
  import LRoutingMachine from '@/components/EventControlPage/LRoutingMachine';
  import MapInfoControl from "@/components/EventControlPage/MapInfoControl";
  import CancelGeoWindow from '@/components/EventControlPage/CancelGeoWindow';
  import ImportGeoZoneWindow from '@/components/EventControlPage/ImportGeoZoneWindow';
