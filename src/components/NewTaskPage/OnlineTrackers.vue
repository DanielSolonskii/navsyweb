<template>
  <l-control position="topleft" class="elevation-10">
    <v-slide-x-transition>
      <button v-show="hideOnlineTrackers" type="button" class="v-btn show-online-trackers"
        @click="hideOnlineTrackers = false">
        <v-icon class="rotate-icon">mdi-menu-open</v-icon>
      </button>
    </v-slide-x-transition>
    <v-slide-x-transition>
      <v-card v-show="!hideOnlineTrackers" class="mx-auto container-" width="400">
          ...
          <v-row class="pb-2">
            ...
            <v-tooltip left>
              <template v-slot:activator="{ on, attrs }">
                <v-btn icon>
                  <v-icon v-bind="attrs" v-on="on" @click.stop="toggleCalendar=true">mdi-map-marker-path</v-icon>
                </v-btn>
              </template>
              <span>История</span>
            </v-tooltip>
        ...
        <template v-if="groupNames.success && !oneList">
          <div v-for="list in filteredgroupsWithMarkers" :key="list.individualId">
            <v-expansion-panels class="expansion-panel-tracker">
              <v-expansion-panel hide-actions>
                <v-expansion-panel-header :style="{ 'border-left': `4px solid #${list.color}`}"
                  class="border-radius-zero filtred-list-label pl-4 pb-2 pt-2 expansion-panel-header-closed">
