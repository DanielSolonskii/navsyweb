<template>
    <v-container fluid class="pa-0 d-flex flex-column h-100">
            ...
            <v-card light max-width="500px" height="100%" rounded="0" class="d-flex flex-column pa-4">
                <div>
                    <v-row align="center" class="ma-0">
                        ...
                        <v-col>
                            <span class="text-h5 font-weight-light">Место</span>
                        </v-col>
                        <v-col>
        ...
        <!-- new place -->
        <v-overlay absolute :value="addNewPLace" class="overlay-form-content">
            <v-card light max-width="500px" height="100%" rounded="0" class="d-flex flex-column pa-4">
                <div>
                    <v-row align="center" class="ma-0">
                        <v-col cols="2">
                            <v-btn icon class="text-center" @click="addNewPLace = false">
                                <v-icon>
                                    mdi-close
                                </v-icon>
                            </v-btn>
                        ...
                        <v-col cols="6">
                            <span class="text-h5 font-weight-light">Новый сотрудник</span><br>
                        </v-col>
                        <v-col>
                            <v-icon @click="changeProfilePicture()" class="place-profile-panel-picture">
                                {{ editedItem.pic }}</v-icon>
                ...
                <v-row class="ma-0 overflow-y-auto" style="min-height: 0" v-if="this.editedItem != {}">
                    <v-form ref="form-garage">
                        <v-row class="ma-0 pl-4 pr-2">
                            <v-col cols="10">
                            <v-row><span>Основная информация</span></v-row>
                                <v-row>
                                    <v-icon class="mr-3 mt-4">mdi-format-color-text</v-icon>
                                    <v-text-field label="Сотрудник" hide-details="auto" v-model="editedItem.name">
                                    </v-text-field>
                                </v-row>
                            </v-col>
                            <v-col cols="10">
                                <v-row>
                                    <v-icon class="mr-3 mt-4">mdi-map-marker</v-icon>
                                    <v-text-field label="Адрес офиса" hide-details="auto" v-model="editedItem.address">
                                    </v-text-field>
                                    <v-icon class="ml-3 mt-4">mdi-map</v-icon>
                                </v-row>
                            </v-col>
                            <v-col cols="10">
                                <v-row>
                                    <v-icon class="mr-3">mdi-text-long</v-icon>
                                    <v-text-field label="Эл. ключ" hide-details="auto" v-model="editedItem.description">
                                    </v-text-field>
                                </v-row>
                            </v-col>
                            <v-col cols="10" class="mt-2">
                ...
                <v-card-actions class="mt-auto">
                    <v-spacer></v-spacer>
                    <v-btn color="grey darken-1" text @click="addNewPLace = false">
                        Отмена
                    </v-btn>
                    <v-btn color="primary darken-1" text @click="addNewPLace = false">
                        Сохранить
                    </v-btn>
                </v-card-actions>
            </v-card>
        ...
        <v-container fluid class="h-100 pa-2">
            <v-row class="ma-0 h-100">
                <v-col class="vehicles-container pa-0 h-100" :class="{'active': openVehicleSidebar}">
                    <v-row justify="space-between" align="center" class="ma-0">
                        ...
                        <v-col cols="auto" class="px-0">
                            ...
                            <v-text-field v-model="search" ref="search-car" label="Поиск сотрудника"
                                placeholder="Поиск сотрудника" dense single-line hide-details
                                :append-icon="showSearchCar ? 'mdi-close' : ''" @click:append="clearSearchCar"
                                :class="{'show': showSearchCar}" class="vehicles-search">
                            </v-text-field>
                    ...
                    <v-card style="height: calc(100vh - 160px)" class="pa-0">
                        <v-row class="ma-0" align="center" style="min-height: 44px; height: 80px">
                            ...
                            <v-col class="py-1 text-center d-flex" style="max-width: 150px">
                                <v-btn icon class="filter-icon-fix" @click="multipleItemsDialog = true">
                                    <v-icon size="22">
                        ...
                        <v-card class="height: calc(100vh - 200px);">
                            ...
                            <!-- dialog -->
                            <!-- dialog multiple items delete start-->
                            <v-dialog v-model="multipleItemsDialog" max-width="290">
                                <v-card class="pa-4">
                                    <v-card-text>
                                    ...
                                    <v-card-actions>
                                        <v-spacer></v-spacer>
                                        <v-btn plain text dense @click="multipleItemsDialog = false">
                                            Отмена
                                        </v-btn>
                                        <v-btn plain text dense @click="deleteItemsHeader">
                                            Удалить
                                        </v-btn>
                                    </v-card-actions>
                                </v-card>
                            </v-dialog>
                            <!-- dialog multiple items delete end-->
                        </v-card>
                    </v-card>
                </v-col>
                <v-col class="pa-0 h-100 d-none" style="max-width: 400px;" :class="{'d-block': openVehicleSidebar}">
                    ...
                    <v-card class="pa-0 ml-4" style="height: calc(100% - 140px)">
                        ...
                        <div class="overflow-y-auto" style="height: calc(100% - 300px)">
                            <div class="pa-4" v-if="itemToShowInformation == null">
                                <h4 class="select-task-text">Выберите запись из списка</h4>
                            </div>
                            <template v-else>
                                <div class="pa-0 ma-0 left-panel-styling">
                                    ...
                                    <div class="pa-4">
                                        ...
                                        <h4>{{ this.itemToShowInformation.department }}</h4>
                                    </div>
                                    <v-divider></v-divider>
                                    <div class="pa-4">
                                    ...
                                    <v-divider></v-divider>
                                    <div class="pa-4">
                                        <v-list-item-subtitle>Маячки</v-list-item-subtitle>
                                        <h4>{{ this.itemToShowInformation.form }}</h4>
                                    </div>
                                    <v-divider></v-divider>
                                    <div class="pa-4">
                                        <v-list-item-subtitle>Форма</v-list-item-subtitle>
                                        <h4>{{ this.itemToShowInformation.official_number }}</h4>
                                    </div>
                                    <v-divider></v-divider>
                                    <div class="pa-4">
    ...
    export default {
        ...
        data: () => ({
            ...
            tasks: [{
                ...
                tracker_name: 'Аристарх',
                official_number: '1',
                department: 'Отдел продаж',
                phone_number: '+7898798',
                email: 'test@mail.ru',
                driver_license_number: '11111',
                driver_license_category: '22222',
                driver_license_due_dat: '12.13.14',
                electric_key: '123123',
