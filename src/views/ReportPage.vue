<template>
  <v-container fluid class="h-100 pa-0 grey lighten-4 relative">
    <v-slide-x-transition>
      <v-card
          v-show="openAddNewReport"
          class="absolute-box up_map_layer ma-3"
          min-width="320px"
          max-width="320px"
          elevation="0"
      ...
      >
        ...
        >
          <v-list-group
              v-for="(group, i) in groups_catalog_reports"
              :key="'group-' + i"
              :value="group"
              class="group-reports"
              color="primary"
              ...
              <template  v-for="(report, i) in group.reports">
                ...
                >
                  <v-list-item-content class="py-0">
                    ...
                    <v-list-item-subtitle style="font-size: 12px">{{report.description}}</v-list-item-subtitle>
                  </v-list-item-content>

                </v-list-item>
                <v-divider
                    v-if="i < group.reports.length - 1"
                    :key="'br-'+i"
                ></v-divider>
              </template>
            </v-list-item-group>
    ...
    <v-slide-x-transition>
      <v-card v-show="selectedNewReport || selectedParamsReport" class="absolute-box ma-3 up_map_layer"
              width="640px"
              style="left: 320px"
      >
        <div class="d-flex h-100 overflow-hidden" >
          ...
          >
            ...
            >
              <v-list-group
                  v-for="(group, i) in grouping_devices"
                  :key="'group-' + i"
                  :value="group"
                  class="group-devises"
                  color="primary"
              ...
              >
                ...
                <v-list-item-group
                    v-model="parametersReport.trackers"
                    multiple
                >
                  <v-list-item
          ...
          <v-card class="h-100 up_map_layer"
                  elevation="0"
                  rounded="0"
                  min-width="320px"
                  max-width="320px"
          ...
          >
            ...
            <v-row class="ma-0">
              <v-col class="px-2 py-1">
                ...
                <v-text-field
                    v-model="parametersReport.title"
                    dense
                    outlined
                    hide-details="auto"
                    color="primary"
            ...
            <v-row class="ma-0">
              <v-col class="px-2 py-1">
                ...
                <v-range-slider
                    v-model="computedControlTimeRange"
                    color="primary"
                    :max="maxTimeControl.unix()"
                    :min="minTimeControl.unix()"
                    step="1800"
                    hide-details
                ></v-range-slider>
                <div class="text-center mt-n2">
                   <span
                       class="indigo--text pointer-on-hover px-1"
            ...
            <v-row v-if="'hide_empty_tabs' in parametersReport.plugin" class="ma-0 px-2">
              <v-checkbox
                  v-model="parametersReport.plugin.hide_empty_tabs"
                  label="Скрывать пустые вклаки"
                  color="primary"
                  hide-details
                  class="mt-0"
              ></v-checkbox>
            ...
            <v-row v-if="'show_seconds' in parametersReport.plugin" class="ma-0 px-2">
              <v-checkbox
                  v-model="parametersReport.plugin.show_seconds"
            ...
            <v-row v-if="'include_summary_sheet_only' in parametersReport.plugin" class="ma-0 px-2">
              <v-checkbox
                  v-model="parametersReport.plugin.include_summary_sheet_only"
                  label="Показывать только общий отчёт"
                  color="primary"
                  hide-details
                  class="mt-0"
              ></v-checkbox>
            ...
            <v-row v-if="'split' in parametersReport.plugin" class="ma-0 pa-2">
              <div style="font-size: small; z-index: 3"
                   class="white ml-2 pa-1 mb-n4"
              >Дополнительно</div>
              <v-card
              ...
              >
                <v-checkbox
                    v-model="parametersReport.plugin.split"
                    label="Разделять стоянками"
                    color="primary"
                    hide-details
                    class="mt-0"
                ></v-checkbox>
                <v-checkbox
                    v-model="parametersReport.plugin.show_idle_duration"
                    label="Показывать длительные стоянки"
                    color="primary"
                    hide-details
                    class="mt-0"
                ></v-checkbox>
                <v-checkbox
                    v-model="parametersReport.plugin.show_coordinates"
                    label="Показывать координаты"
                    color="primary"
                    hide-details
                    class="mt-0"
                ></v-checkbox>
                <v-checkbox
                    v-model="parametersReport.plugin.filter"
                    label="Использовать умный фильтр"
                    color="primary"
                    hide-details
                    class="mt-0"
                ></v-checkbox>
                <v-checkbox
                    v-model="parametersReport.plugin.group_by_driver"
                    label="Группировать по водителям"
                    color="primary"
                    hide-details
                    class="mt-0"
                ></v-checkbox>
            ...
            <v-row class="ma-0 pa-2 text-center">
              ...
              <v-btn
                  ...
                  width="160"
                  class="ml-auto"
                  :disabled="!parametersReport.trackers.length"
                  color="primary"
                  rounded
                  style="color: white"
                  @click="generationNewReport()"
              >
                Сгенерировать
              </v-btn>
            </v-row>
    ...
    <v-row class="h-100 ma-0">
      <v-col class="h-100 relative reports_view" style="min-width: 344px">
        <v-card height="100%" width="320px" class="">
          <v-toolbar
              dense
              dark
              color="primary"
            ...
            >
              ...
              >
                <template v-for="(item , i) in listReports">
                  ...
                  >
                    <v-progress-linear
                        v-if="loadNewReport === item.id"
                        absolute
                        color="indigo lighten-4"
                        height="100%"
                        :style="{left: '-' + progressLoadNewReport + '%'}"
                        style="width: 200%"
                        value="100"
                        striped
                    ></v-progress-linear>
                    <v-list-item-content class="pa-0" style="z-index: 3">
                      ...
                      <v-list-item-subtitle v-if="loadNewReport !== item.id">
                        Сформирован {{ item.created | formatDateCreatedReport}}
                      </v-list-item-subtitle>
                    </v-list-item-content>
                    <v-list-item-action>
                      <div>
                        <v-icon
                            class="d-none pointer-on-hover list-item-edit__icon"

                        >mdi-file-pdf-box</v-icon>
                        <v-icon
                            class="d-none ml-1 pointer-on-hover list-item-edit__icon"
                            @click.prevent.stop="openParametersReport(item)"
                        >mdi-cog</v-icon>
                      </div>
      ...
      <v-col class="report_view h-100" style="min-width: calc( 100% - 344px)">
        <v-sheet v-if="selectedReport && !report" height="100%" width="100%" class="transparent">
          <v-row justify="center" class="h-100 ma-0" align="center">
             <v-progress-circular
            :size="70"
            :width="5"
            color="grey"
        ...
        <v-card v-if="selectedReport && report" height="100%" width="100%">
          <v-row class="ma-0 primary" align="center">
            <v-col style="max-width: 50px" class="pa-2">
              <v-btn
                  icon
                  color="blue darken-3"
            ...
            <v-col class="pa-2">
              <h4 class="text-center white--text">{{selectedReport.title}}</h4>
            </v-col>
          </v-row>
          <v-row class="ma-0" style="height: calc( 100% - 55px )">
            <travel-report :report="report"></travel-report>
          </v-row>
        </v-card>
      </v-col>
    </v-row>
...
export default {
  ...
  computed: {
    ...
    computedControlTimeRange: {
      get() {
        return [moment(this.parametersReport.time_filter.from).unix(), moment(this.parametersReport.time_filter.to).unix()]
      },
      set(range) {
  ...
  methods: {
    generationNewReport() {
      ...
      ReportsService.createReport(this.parametersReport).then((data) => {
        this.report = data
  ...
  created() {
    ...
    ReportsService.createReport('start').then((data) => {
      this.report = data
    })
  },
  data: () => ({
...
::v-deep .v-list-item--dense.v-list-item--two-line, .v-list--dense .v-list-item.v-list-item--two-line {
  min-height: 50px;
}
