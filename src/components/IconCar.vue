...
<script>
import {optionsMerger, propsBinder, findRealParent} from 'vue2-leaflet'
  ...
  mounted() {
    ...
    this.observer = new MutationObserver((mutaions) => {
      ...
      mutaions.forEach(mutaion => {
        if (mutaion.attributeName === 'style' || mutaion.attributeName === 'class') {
          //this.parentContainer.mapObject.getElement().querySelector();
          console.log(mutaion.target.classList.value)
          let qs = mutaion.target.classList.value;
          let elem = this.$el.querySelector(qs);
          //elem.
          //this.$el.querySelector('.')
          console.log(mutaion, this.$el, elem)
        } else {
          //console.log(mutaion)
          flagSwap = true
        }
