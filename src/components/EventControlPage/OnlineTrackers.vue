        ...
        <template v-if="groupNames.success && !oneList">
          <div class="expansion-panel-tracker" v-for="list in filteredgroupsWithMarkers" :key="list.individualId">
            <div v-for="item in list.marker" :key="item.id" class="bottom-light-box-shadow pt-1px">
              <v-list-item class="pt-3 expansion-panel-tracker-item align-center" @click="clickHandlerTest(item.id)">
                <template>
                  <v-list-item-icon>
                    <v-icon>mdi-speedometer</v-icon>
                  </v-list-item-icon>
                  <v-list-item-content class="pa-0" style="display: flex; flex-direction: column; align-items: left">
                    <v-list-item-subtitle class="item-label" v-if="item.speed">Превышение скорости - {{ item.speed }}
                      км/ч
                    </v-list-item-subtitle>
                    <v-list-item-title class="item-label">{{ item.title }}</v-list-item-title>
                    <v-list-item-subtitle class="item-label" v-if="item.time">{{ item.time }}</v-list-item-subtitle>
                  </v-list-item-content>
                </template>
              </v-list-item>
            </div>
