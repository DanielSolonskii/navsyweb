<template>
    <v-slide-x-transition>
        <l-control position="topleft" disableClickPropagation disableScrollPropagation>
            <div class="d-flex">
                <v-btn @click="$router.push('schedules')" class="left-panel-go-back-btn white elevation-4 mr-3" icon
                    color>
                ...
                <v-card ref="form" class="elevation-10 new-task-left-panel">
                    ...
                    <v-dialog v-model="dialog" class="v-dialog-left-panel pa-3">
                        <template v-slot:activator="{ on, attrs }">
                            <div v-bind="attrs" v-on="on" class="d-flex mt-6 ml-4 mb-5">
                                <v-icon class="mr-4">mdi-calendar</v-icon>
                                <div class="d-flex flex-column"><span class="small-grey-text">Повторять</span>
                                    <b v-if="triggerScheduleVariant === 'month-tab'">Каждый месяц (<span
                                            v-for="(day,index) in scheduledDays" :key="day.id">{{ day.text }}<span
                                                v-if="index != scheduledDays.length - 1">, </span></span>) числа</b>

                                    <b v-else>
                                        <template v-if="!everyDay">
                                            Каждую неделю (<span v-for="(day,index) in scheduledDays"
                                                :key="day.id">{{ day.text }}<span
                                                    v-if="index != scheduledDays.length - 1">,
                                                </span></span>)
                                        </template>
                                        <template v-else-if="everyDay">
                                            Каждый день
                                        </template>
    ...
    export default {
        ...
        methods: {
            taskActiveEdit(index) {
                ...
                this.$root.$emit("scheduled-active-task-point", this.taskPoints[index]);
                // return this.taskPoints[index];
            },
            addNewPoint() {
                // const curentIndex = this.taskPoints.length - 1;
                // this.taskActiveEdit(curentIndex);
                this.$root.$emit("add-new-point-scheduled-btn-pressed", true);
            },
            ...
            deleteTaskPoint(index) {
                ...
                this.$root.$emit('scheduled-task-deleted', this.taskPoints);
            },
